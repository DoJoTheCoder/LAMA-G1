...
export default function GetLoan() {
  ...
  function applyLoanSubmit(event) {
    ...
    if (itemCategory === "") {
      ...
      axios
        .post("http://localhost:8080/addNewLoan", sendBody)
        .then((response) => {
          console.log(response.data);
          if (response.data === "ok") {
            alert("Added Loan successfully!");
            navigate("/current-loans");
          } else alert("Failed to add loan! Try again later.");
        })
        .catch((error) => {
          alert("Error: " + error);
        });
  ...
  const getItemCategoryData = () => {
    axios
      .get("http://localhost:8080/getAllItemCategories")
      .then((response) => {
        setItemCategoryArray(response.data);
      })
      .catch((error) => {
  ...
  const getItemMakeData = (itemCatSelected) => {
    ...
    if (itemCatSelected !== "Null") {
      ...
      axios
        .get("http://localhost:8080/getItemMakes/" + itemCatSelected)
        .then((response) => {
  ...
  const getItemDescriptionData = (itemMakeSelected) => {
    ...
    if (itemMakeSelected !== "Null") {
      ...
      axios
        .get(
          "http://localhost:8080/getItemDescriptions/" +
            itemCategory +
            "/" +
            itemMakeSelected
        )
  ...
  const setItemValueField = (itemDescSelected) => {
    ...
    if (itemDescSelected !== "Null") {
      ...
      axios
        .get("http://localhost:8080/getItemValue/" + itemDescSelected)
        .then((response) => {
        ...
        <form id="newLoanForm" onSubmit={(e) => applyLoanSubmit(e)}>
          ...
          <div className="input-group flex-fill mb-3">
            <div className="input-group-prepend">
              <label className="input-group-text" htmlFor="itemCategoryInput">
                Item Category
              </label>
            </div>
            <select
              ...
              defaultValue="Null"
              onChange={(e) => {
                getItemMakeData(e.target.value);
          ...
          <div className="input-group mb-3">
            <div className="input-group-prepend">
              <label className="input-group-text" htmlFor="itemMakeInput">
                Item Make
              </label>
            </div>
            <select
              ...
              defaultValue="Null"
              onChange={(e) => {
                getItemDescriptionData(e.target.value);
          ...
          <div className="input-group mb-3">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setItemValueField(e.target.value);
              }}
              disabled
            >
              <option key="None" value="Null">
              ...
              {itemDescArray.map((itemDesc, index) => (
                <option key={itemDesc} value={itemDesc}>
                  {itemDesc}
                </option>
              ))}
            </select>
